...
#include "pawn.h"
extern Game *game;
King::King(QString team, QGraphicsItem *parent)
...
void King::moves()
{
    ...
    if (col > 0 && row > 0
        && !(game->collection[row - 1][col - 1]->getChessPieceColor() == team)) { //up left
    ...
    if (col > 0 && row < 7
        && !(game->collection[row + 1][col - 1]->getChessPieceColor() == team)) { //down left
        location.append(game->collection[row + 1][col - 1]);
        game->collection[row + 1][col - 1]->setColor(Qt::darkRed);
        if (location.last()->getHasChessPiece()) {
            location.last()->setColor(Qt::red);
    ...
    if (col < 7 && row < 7
        && !(game->collection[row + 1][col + 1]->getChessPieceColor() == team)) { //down right
        location.append(game->collection[row + 1][col + 1]);
        game->collection[row + 1][col + 1]->setColor(Qt::darkRed);
        if (location.last()->getHasChessPiece()) {
            location.last()->setColor(Qt::red);
    ...
    for (size_t i = 0, n = static_cast<size_t>(pList.size()); i < n; i++) {
        if (pList[static_cast<int>(i)]->getSide() != this->getSide()) {
            ...
            for (size_t j = 0, n = static_cast<size_t>(bList.size()); j < n; j++) {
                Pawn *c = dynamic_cast<Pawn *>(pList[static_cast<int>(i)]);
                if (c)
                    continue;
